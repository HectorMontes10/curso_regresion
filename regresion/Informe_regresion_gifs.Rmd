...
estimador_betas <- function(x,y){
    ...
    coeficientes <- lm(y~x)$coefficients
...
df_rectas <- data.frame(x=numeric(), y=numeric(), numero_muestra = numeric())



for (i in 1:10) {
  
    muestra <-  generador_muestra(n,vector_x,sigma, beta_0,beta_1)
    betas_estimados <-  estimador_betas(muestra$x, muestra$y)
  df <- data.frame(x = vector_x,
                   y = betas_estimados$beta_0 + betas_estimados$beta_1 * vector_x,
...
ggplot(data = df_rectas, aes(x=x, y=y, group = numero_muestra))+
    geom_line(aes(color = numero_muestra))
