...
Observe el siguiente gráfico en el que hemos pintado las distancias
que hay entre cada dato $y_i$ observado y el promedio histórico (valor predicho $\bar{y}$ por el modelo ingenuo)
...
Expliquemos qué implica la ecuación anterior haciendo ciertas anotaciones de gran
relevancia práctica para entender **cómo los estadísticos piensan al
momento de plantear modelos**:
...
aleatoria normal con media $\mu=0$ y desviación estándar $\sigma$. Conviene aclarar que esto es una suposición, no implica que efectivamente los errores vayan a cumplir tal supuesto. 
...
$y_i$ y $x_i$ observado. Vimos que esto ocurrión con el modelo ingenuo y por supuesto ocurre también para un modelo no ingenuo. La idea es entonces reducir la magnitud de estos errores al mínimo posible y el par de valores $\beta_1$ y $\beta_2$ que así lo logren será nuestra elección óptima de cara al objetivo de minimización del error.
...
Por lo pronto vamos a proceder de manera más intuitiva, dejando de lado el formalismo matemático, y vamos a ejemplificar un posible modelo ajustado usando
valores $\beta_0=700$ y $\beta_1 = 1.2$. Hemos elegido estos valores por
simple inspección visual así que no esperamos que sean óptimos en ningún
sentido, veamos:
...
elección de los betas. Usaremos entonces la métrica MSE(Mean Squared Errors) dada por:
...
$MSE_{\text{NO ingenuo}}$. Dicho de otra manera los pares de valores  $(x_i, y_i)$
**se comportan como constantes** a lo largo de mi proceso de minimización. 
...
$$E(y) = E(\beta_{0} + \beta_{1} * x + e)$$
Pero como $\beta_0$ y $\beta_1$ son constantes, tenemos que:

$$E(y) = E(\beta_0) + E(\beta_1*x) + E(e)$$
Así que:

$$E(y) = \beta_0 +\int_{-\infty}^{\infty}\beta_1 *xf(x)dx + E(e)$$
$$E(y) = \beta_0 + \beta_1*\int_{-\infty}^{\infty}xf(x)dx + E(e)$$
Y como $\int_{-\infty}^{\infty}xf(x)dx$ es por definición $E(x)$ y además $E(e) = 0$ por diseño (de acuerdo con el supuesto usado para la distribución del error), entonces:

$$E(y) = \beta_0 + \beta_1*E(x)$$
Dicho de otra manera, la recta del modelo debe pasar por el punto
$(E(x), E(y))$ !!
...
estos valores son estimados de los correspondientes  $\beta_0$ Y $\beta_1$ del modelo
teórico subyacente. En conclusión: la mejor recta estimada deberá pasar
por $(\bar{x}, \bar{y})$, y por lo tanto hay una relación atando a los
betas estimados:
...
\end{equation}
$$
Es decir que dando valores a $\color{red}{\widetilde\beta_1}$ podremos
obtener valores para $\color{red}{\widetilde\beta_0}$ y en últimas
dibujar diversas rectas en el plano, con el fin de establecer cuál de
ellas minimiza el $MSE_{\text{NO ingenuo}}$. 
...
$$
Ahora, usando el hecho de que el lado derecho de la ecuación anterior es por definición $cov(x,y)$, es decir, la covarianza entre la variable regresora $x$ y la variable respuesta $y$, así como la expresión dada en $\ref{eq:sigma_x}$ tenemos:
...
## 1.13 Representación funcional del modelo y comprobación de supuestos.
...
Pero, aunque nos hemos esforzado por encontrar las mejores estimaciones de los betas para minimizar la función de pérdida, esto no garantiza que nuestro modelo induzca errores normales, por tal motivo conviene revisar lo siguiente:


☁ Recordemos que:
Cuando planteamos el modelo, tuvimos que suponer algunas cosas, entre ellas, que la distribución de los errores sería *normal*, es decir, que los errores seguirian una distribución *normal estándar*. Por lo tanto, ahora que tenemos construído nuestro modelo, es necesario que validemos la suposición hecha.

🤔 ¿Cómo hacerlo? ❔

Tabla comparativa de las ventajas y desventajas de las tres pruebas de normalidad: Kolmogorov-Smirnov, Lilliefors y Shapiro-Wilk.
...
🤔 Y si el texto anterior dice: "el conocimiento previo sobre la distribución de los datos" 🤔 
¿Será necesario representar la distribución en un gráfico? 
¿Cómo funciona la prueba Lilliefors?


```{r}
ajuste <- fitdist(mod1$residuals, "norm",method='mme')
...
#df_residuals <- df_residuals[order(df_residuals$residuals),]


cdf_teorica <- pnorm(df_residuals$residuals,
                      mean = ajuste$estimate[1],
                      sd = ajuste$estimate[2])


# Estoy grabando una función teorica existente en R
# y asignandola a un nombre llamado funcion_teorica.
funcion_teorica <- pnorm

# Esta cálcula la función de distribución acumulada
# con los datos de los residuales. 
# Matematicamente esto es una función a Trozos.
# Al ser una función está a la espera de un parámetro para poderse evaluar.
cdf_empirica <- ecdf(df_residuals$residuals)

# Como deseo calcular la diferencia entre lo calculado entre ambas funciones. Voy a crear una función.
funcion_diferencia <- function(x) {
  cdf_empirica(x) - funcion_teorica(x, mean = ajuste$estimate[1], sd = ajuste$estimate[2])
}

funcion_diferencia_der <- function(x) {
   seq(1,length(x),1)/length(x) - funcion_teorica(x, mean = ajuste$estimate[1], sd = ajuste$estimate[2])
}

funcion_diferencia_izq <- function(x) {
  funcion_teorica(x, mean = ajuste$estimate[1], sd = ajuste$estimate[2]) - seq(0,length(x),1)/length(x)
}


diferencia_resultados <- sapply(df_residuals$residuals, funcion_diferencia)

diferencia_resultados_der <- sapply(df_residuals$residuals, funcion_diferencia_der)

diferencia_resultados_izq <- sapply(df_residuals$residuals, funcion_diferencia_izq)

pos_estadistico_der <- which.max(abs(diferencia_resultados_der))
pos_estadistico_izq <- which.max(abs(diferencia_resultados_izq))

estadistico_der <- diferencia_resultados_der[pos_estadistico_der]
estadistico_izq <- diferencia_resultados_izq[pos_estadistico_izq]

estadistico <- max(estadistico_izq,estadistico_der)

lillieforce_test <- lillie.test(df_residuals$residuals)
lillieforce_test$statistic
...
w1 <-  which(diferencia_resultados_der == lillieforce_test$statistic)
w2 <-  which(diferencia_resultados_izq == lillieforce_test$statistic)
w3 <- which(abs(diferencia_resultados)== lillieforce_test$statistic)

print(w1)
print(w2)
print(w3)

estadistico_mod <- estadistico*(sqrt(nrow(df_residuals)) - 0.01 + (0.85/sqrt(nrow(df_residuals))))


ks.test(df_residuals$residuals,"pnorm")
...
#🛑🛑🛑
x_seg <- df_residuals$residuals[estadistico]
y_sup_seg <- cdf_teorica[estadistico]
...
ggplot(data.frame(x = df_residuals$residuals), aes(x = residuals)) +
  geom_density(color = "blue", fill = "lightblue", alpha = 0.5) +
  stat_function(fun = pnorm, args = list(mean = ajuste$estimate[1],
                                         sd = ajuste$estimate[2]),
                color = "red", size = 1) +
  ...
  annotate("text", x = x_seg , y = (y_inf_seg + y_sup_seg)/2, 
           label = paste0("Máxima Separación=",
                          format(lillieforce_test$statistic,
                                 digits=5), sep=''),
           color = "purple", size = 3, hjust = -0.05, vjust = 0) +
  coord_cartesian(xlim = c(min(df_residuals$residuals), max(df_residuals$residuals))) +
  xlab("Ingresos") +
  ylab("Probabilidad acumulada") +
  labs(title = "Esquema de funcionamiento de la prueba de Kolmogorov-Smirnov") +
  theme_minimal()
...
Vamos probar la normalidad de los errores utilizando las pruebas de Kolmogórov-Smirnov y la de Shapiro–Wilk.


En resumen, la elección entre la prueba de Lilliefors y la prueba KS estándar depende de tus objetivos y de las suposiciones sobre la distribución de tus datos. Si estás interesado específicamente en verificar la normalidad de tus datos, la prueba de Lilliefors puede ser apropiada, especialmente para muestras pequeñas. Sin embargo, si no tienes una distribución específica en mente o si deseas verificar otras distribuciones, la prueba KS estándar es una opción más versátil.
En cualquier caso, es importante recordar que ninguna prueba es infalible, y es fundamental interpretar los resultados en el contexto de tus objetivos de análisis y la naturaleza de tus datos.
...
ks.test(mod1$residuals, y = pnorm(mod1$residuals,mean = 0, sd = sqrt(sigma_2)))
...
Restricción a distribución normal: La principal desventaja de la prueba de Lilliefors es que está restringida a la comprobación de la normalidad. No es adecuada para verificar si los datos se ajustan a otras distribuciones diferentes a la normal. En cambio, la prueba KS estándar es más versátil y se puede aplicar a cualquier distribución teórica.
...
❗ Menos potente: En comparación con la prueba KS estándar, la prueba de Lilliefors puede ser menos potente para detectar desviaciones de la normalidad en muestras grandes, especialmente si los datos no siguen una distribución estrictamente normal. La prueba KS estándar es más robusta en este sentido.

🤔 ¿Y esto será conveniente?

Supuestos: Al igual que con la prueba KS estándar, la prueba de Lilliefors asume independencia y continuidad de los datos, lo que puede no ser adecuado para todos los tipos de datos.
...
- ¿Si al plantear un modelo alternativo que captura mejor el comportamiento del error?
