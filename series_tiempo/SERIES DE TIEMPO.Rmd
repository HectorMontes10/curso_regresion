...
```{r}
model2<-arima(df_variaciones$variaciones, order=c(0,0,2))
summary(model2)
```
...
test_ind_1 <-Box.test(model$residuals, lag=6, type="Ljung-Box")
test_ind_2 <- Box.test(model2$residuals, lag = 6, type="Ljung-Box")
...
- Son muy competitivas entre si.

```{r}
library(nortest)
normalidad<-lillie.test(model2$residuals)
print(normalidad)
hist(model2$residuals)
graph_lilliefors(data.frame(residuals=model2$residuals),for_residuals = FALSE)
...
predecir_sem_1 <- predict(model2, n.ahead=7,interval="confidence", level=0.95)
print(predecir_sem_1)

# AirPassengers
```

```{r}
obj_pronostico<-forecast(model2)
obj_pronostico$mean
autoplot(obj_pronostico)
```

# 9. CASO APLICADO DATASET DE AGUACATE

1. Calcule ACF Y PACF para el precio del dolar usando las funciones acf y pacf de R
2. Estudie el comportamiento de esos gráficos y compare con la tabla del capítulo 5 y deduzca cuál es el orden del modelo.
3. Estime el modelo usando la funcion arima y el argumento order, donde order fue identificado en el paso 2.
4. Extraigan los residuales del modelo y apliquen prueba de Ljung-box a 6 rezagos y prueba de normalidad.
5. Si el modelo pasa las pruebas construya el pronóstico de 7 pasos adelante, si no devuélvase al punto 1 y pruebe con un nuevo modelo.

```{r}
# Paso 1:
acf(dolar$precio_dolar)
pacf(dolar$precio_dolar)


dolar <- dolar %>% 
  mutate(var_dolar=(precio_dolar-lag(precio_dolar))/lag(precio_dolar)) %>% 
  filter(!is.na(var_dolar))

dolar %>% 
  ggplot(aes(x= fecha, y = var_dolar, group = 1))+
  geom_line()

acf(dolar$var_dolar)
pacf(dolar$var_dolar)

#Paso 2: Consultar tabla cap 5
mi_orden =c(0,0,1)

#Paso 3:
model_dolar<-arima(dolar$var_dolar, order=mi_orden)

#Paso 4:

Ljung_box <- Box.test(model_dolar$residuals, lag=6, type="Ljung-Box")
print(Ljung_box)
Lillie<-lillie.test(model_dolar$residuals)
graph_lilliefors(data.frame(residuals=model_dolar$residuals),for_residuals = FALSE)

modelo_dolar_2<-auto.arima(dolar$var_dolar)
summary(modelo_dolar_2)

#Paso 5: Método 1
predecir_sem_1 <- predict(model_dolar, n.ahead=7,interval="confidence", level=0.95)
print(predecir_sem_1)
#Paso 5: Método 2
forecast(model_dolar)
obj_predicciones<-forecast(model_dolar)
obj_predicciones$mean
#Para obtener los valores inferiores y superiores del intervalo de confianza de mi pronóstico 
obj_predicciones$lower
obj_predicciones$upper

#Para graficar incluyendo el intervalo de confianza
autoplot(obj_predicciones)
```
