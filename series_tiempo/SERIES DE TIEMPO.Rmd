...
```{r}
acf_or <- acf(hass_dia$precio_kg)
data.frame(
  lag = acf_or$lag,
  acf = acf_or$acf) %>% head(10) %>% ftable()
```
...
```{r}
pacf_or <- pacf(hass_dia$precio_kg)
data.frame(
  lag = pacf_or$lag,
  pacf = pacf_or$acf) %>% head(10) %>% ftable()
...
paste0("Esta es la autocorrelación parcial a 1 lag calculada con mi fórmula: ",
       phi_33, " y esta es la calculada con r: ",pacf_or$acf[3])
```
