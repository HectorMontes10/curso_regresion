...
A continuación un ejemplo de cómo estimar la ACF sobre los datos de aguacate usando la función acf que ya viene implementada en R.
...
Veamos como extraer tales estimados usando R
...
#Escriba acá el código para comparar el resultado de la prueba antes y después de esta transformación
...
<figcaption>Img 3: Ejemplos de ACF para procesos AR(1) a) $\phi=0.8$ b) $\phi=-0.8$</figcaption>
</figure>
...
<figcaption>Img 3: Ejemplos de ACF para procesos AR(2) a) $\phi=0.5, \phi_2=0.3$ b) $\phi_1=1,\phi_2=-0.89$</figcaption>
</figure>
...
<figcaption>Img 1: Proceso MA(1) simulado, $\theta_1=0.8$ y su f.a.c.</figcaption>
</figure>
...
<figcaption>Img 1: Proceso ARMA(1,1) simulado, $\phi=0.8$, $\theta_1=0.3$ y f.a.c f.a.c.</figcaption>
</figure>
...
hass_mes %>% 
  mutate(n_registro = as.numeric(rownames(.))) %>% 
  ggplot(aes(x= n_registro, y = precio_aguacate_kg, group = 1))+
  geom_line()

acf_mensual <- acf(hass_mes$precio_aguacate_kg)
acf_mensual$acf
pacf_mensual <-pacf(hass_mes$precio_aguacate_kg)
pacf_mensual$acf
```

Como notamos tendencia vamos a estacionarizar la serie contemplando los cambios porcentuales
...
H0δ=0
 → Existe raíz unitaria, xt $\rightarrow$ La serie no es estacionaria.

H0δ≠0
 → No existe raíz unitaria,xt  $\rightarrow$ La serie es estacionaria.

Si:
τ  − calculado en valor absoluto > τ−crítico, en valor absoluto: Se rechaza la H0.

τ  − calculado en valor absoluto < τ−crítico, en valor absoluto: Se acepta la H0.
...
⚠️ Importante:
Acà estamos partiendo del supuesto de que la serie tiene tendencia estocástica, pero si este supuesto falla y la serie en realidad tiene tendencia deterministica, las conclusiones de la prueba DICKER FULLER pueden ser engañosas.

Porque tendríamos que entrar a analizar:
Cuál es la diferencia entre tendencia estocástica y tendencia deterministica.

Estocastica se refiere a que el crecimiento de la serie es aparente, en realidad la serie fluctua alrededor de un valor medio, cuando se considera en diferencias (saltos de un periodo al siguiente).

La tendencia deterministica si habla de un pedazo de la serie que efectivamente está teniendo un crecimiento estructural producto de una causa externa que así lo está garantizando.

El modelado del inflación
Si una politica monetaria tiene una meta para llevar la inflación a un valor determinado...

Esta prueba también se puede aplicar con la función adf.test de la libreria tseries
...
hass_mes <-  hass_mes %>% mutate(diferencias = precio_aguacate_kg - lag(precio_aguacate_kg)) %>% 
                  filter(!is.na(diferencias))


df <- ur.df(hass_mes$diferencias, type = "none", lags = 0)
...
df_variaciones$pronosticos <- model$fitted
```
